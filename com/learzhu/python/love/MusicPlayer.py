...
# pip install libmagic  not work
# 在安装eyeD3时，可用pip进行安装，如果在import时出现报错ImportError: failed to find libmagic. Check your installation。解决办法是安装python_magic_bin，python_magic_bin可在网站https://pypi.org/project/python-magic-bin/0.4.14/上下载whl文件，然后直接pip安装即可。

# pip install eyeD3
import time, eyed3, threading
# : name 'filedialog' is not defined
...
def openfile(index=[1]):
    ...
    # root.withdraw askopenfilenames
    # 此处特别注意需要全部引用
    filenames = tkinter.filedialog.askopenfilenames(title="音乐播放器",
                                            filetypes=[("mp3文件", "*.mp3"), ("WMA文件", "*.wma"), ("WAV文件", "*.wav")])
    if filenames:
